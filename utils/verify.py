...
# 验证码静态文件地址
CODE_TEMP_DIR = os.path.join(settings.BASE_DIR, 'static', 'verify')
...
class VerifyCode(object):
    ...
    def _get_worlds(self):
        ...
        读取默认的单词表
        """
        file_path = os.path.join(CODE_TEMP_DIR, 'worlds.list')
        f = open(file_path,'r')
    ...
    def _yield_code(self):
        ...
        def world():
            ...
            code = random.sample(list('ACDEFGHJKLMNPQRSTUVWXYabcdefghijkmnpqrstuvwxy34679'), 4)
            code = str(''.join(code))
    ...
    def display(self):
        ...
        # 验证码字体颜色
        self.font_color = ['black', 'darkblue', 'darkred', 'brown', 'green', 'darkmagenta', 'cyan', 'darkcyan']

        # 随即背景颜色
        self.background = (random.randrange(230, 255), random.randrange(230, 255), random.randrange(230, 255))

        # 字体文件路径
        self.font_path = os.path.join(CODE_TEMP_DIR, 'timesbi.ttf')
        ...
        # 画随机干扰线,字数越少,干扰线越多
        if self.type == 'world':
            c = int(8 / len(self.code) * 2) or 150
        elif self.type == 'number':
            c = 4

        for i in range(random.randrange(c - 2, c)):
            # line_color = (random.randrange(205, 255), random.randrange(205, 255), random.randrange(0, 255))
            line_color = random.choice(self.font_color)
            xy = (
